...
void CEditor::SaveZone()
{
  ...
  for(size_t x=0; x<zoneToEdit->TileMap.size(); x++)
  {
    ...
    ofs << "EditorInterface.addGroundTile( "
        << curTile.x_pos << ", " << curTile.y_pos << ", "
        << "MapData.DontSave."
  ...
  /* sort the environmentmap based on the Y-axis.
     quick-fix for height positions.
     should have another workaround later. */
  std::sort(zoneToEdit->EnvironmentMap.begin(),
            zoneToEdit->EnvironmentMap.end());
  ...
  for(size_t x=0; x<zoneToEdit->EnvironmentMap.size(); x++)
  {
    ...
    ofs << "EditorInterface.addEnvironment( " << curEnv.x_pos << ", "
        << curEnv.y_pos << ", " << curEnv.z_pos << ", "
        << "MapData.DontSave."
    ...
    if( curEnv.transparency != 1 ||
        ...
        curEnv.blue         != 1 )
    {
      ofs << "EditorInterface.adjustLastRGBA( " << curEnv.red << ", "
          << curEnv.green << ", " << curEnv.blue << ", "
          << curEnv.transparency << " );" << std::endl;
    }
    if( curEnv.x_scale != 1 || curEnv.y_scale != 1 )
    {
      ofs << "EditorInterface.adjustLastScale( " << curEnv.x_scale << ","
          << curEnv.y_scale << " );" << std::endl;
    }
  }
  ofs.close();
  ...
  for(unsigned int x=0; x<zoneToEdit->CollisionMap.size(); x++)
  {
    ...
    ofs << "EditorInterface.addCollisionRect( "
        << curCollision.CR.x << ", " << curCollision.CR.y << ", "
        << curCollision.CR.w << ", " << curCollision.CR.h << " );"
        << std::endl;
  }
  ofs.close();
...
void CEditor::updateAdjacencyList()
{
  ...
  {
  case 1: // environment
    EditorInterface::getTileSet()->getAllAdjacentTiles( zoneToEdit->EnvironmentMap[objectedit_selected].tile,
                                                        curAdjacentTiles,
                                                        curAdjacencyOffsets );
    ...
    break;
  case 2:
    EditorInterface::getTileSet()->getAllAdjacentTiles( zoneToEdit->ShadowMap[objectedit_selected].tile,
                                                        curAdjacentTiles,
                                                        curAdjacencyOffsets );
...
void CEditor::DrawEditFrame(sEnvironmentMap* editobject)
{
  ...
  DrawingHelpers::mapTextureToRect( interfacetexture.getTexture(3),
                                    editobject->x_pos,
                                    editobject->tile->texture->getTexture(0).width,
                                    editobject->y_pos,
                                    editobject->tile->texture->getTexture(0).height );
  glPopMatrix();
  ...
  else // adjacencyModeEnabled
  {
    ...
    DrawingHelpers::mapTextureToRect( interfacetexture.getTexture(3),
                                      ...
                                      editorFocus->getY()+
                                      (Configuration::screenHeight/2)-200,
                                      200 );


    // set the color, transparency, scale and then draws the object we are editing
    glPushMatrix();
    glScalef(editobject->x_scale,
             editobject->y_scale,1.0f);
    glColor4f(editobject->red,
              editobject->green,
              editobject->blue,
              editobject->transparency);
